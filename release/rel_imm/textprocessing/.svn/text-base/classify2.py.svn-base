...
vaccine = ['dtp', 'dpt', 'dpt-hb', 'dtp-hb', 'DPT-HB-Hib', 'DTP-HB-Hib', 'BCG', 'polio', 'campak', 'pentavalen', 'pentavalent']
...
filters = [immunization, new_vaccine, disease, outbreak, symptom, halal_haram, vaccine, pentavalent]
...
def determine(res):
   ...
   # symptom + vaccine -> SE
   if output[2]==0 and (res[4]==1 and res[6]==1):
      output[2] = 1
...
def process(a_json):
   ...
   with codecs.open(a_json,'rU','utf-8') as f:
...
if __name__ == '__main__':
   if len(sys.argv) <> 2:
      sys.stderr.write('[usage] %s (text file with the list of jsons)\n')
      exit()

   PARAM_input = sys.argv[1]

   cnt = 0
   for f in open(PARAM_input):
